...
pub fn decode(encoded_image: Qoi) -> DynamicImage {
    ...
    while processed_pixels < header.width * header.height {
        ...
        if b1 >> 6 == 0b00 {
            ...
            let dir = (b1 & 0b0011_0000) >> 4;
            let offset_stored = b1 & 0b0000_1111;
            let offset = if offset_stored >= 0 {
                offset_stored + 1
            } else {
                offset_stored
            };
            let (nbr_x, nbr_y) = if (dir & 0b10) == 0 {
                if (dir & 0b01) == 0 {
                    (x + usize::from(offset), y)
                } else {
                    (x - usize::from(offset), y)
                }
            } else {
                if (dir & 0b01) == 0 {
                    (x, y + usize::from(offset))
                } else {
                    (x, y - usize::from(offset))
                }
            };

            let src_pixel = prev_frame[nbr_x + nbr_y * usize::from(header.width)];
